...
train_noy <- vowel.train[,-1];
test_noy <- subset(vowel.test,select=-c(y));
test_noy <- vowel.test[,-1];
...
if (!exists("rfFit")) {
  ...
  start.time <- Sys.time()
  rfFit <- train(vowel.train$y~ .,data=train_noy,method="rf",prox=TRUE)
#  rfFit <- train(vowel.train$y~ .,data=train_noy,method="rf",prox=TRUE, trControl=trainControl(method="cv"), number=3)
  end.time <- Sys.time()
...
if (!exists("gbmFit")) {
  ...
  start.time <- Sys.time()
  gbmFit <- train(vowel.train$y~ .,data=train_noy,method="gbm",verbose=FALSE)
  end.time <- Sys.time()
  time.taken <- paste( as.numeric(round(end.time - start.time,3)*1000), "milliseconds");
  print(time.taken);
...
predRF <- predict(rfFit,test_noy);
predGBM <- predict(gbmFit,test_noy);
